...
function api(
    ...
    switch (req.params.option) {
        ...
        if (
          ...
          if (!hashes.find((hash) => hash === req.body.params.hash)) {
            ...
            const blockNumber = parseInt(
              (await bhashDB.get(req.body.params.hash)).toString("hex"),
              16
            ).toString();
      ...
      case "get_blockByNumber":
        if (
          ...
          if (
            ...
            const block = [
              ...(await blockDB.get(req.body.params.blockNumber.toString())),
            ];
      ...
      case "get_balance":
        if (
          ...
          const dataFromTarget = deserializeState(
            await stateDB.get(req.body.params.address)
          ); // Fetch target's state object
      ...
      case "get_transactionByTxHash":
        if (
          ...
          try {
            ...
            const [blockNumber, txIndex] = indexPair
              .split(" ")
              .map((item) => parseInt(item));

            const block = Block.deserialize([
              ...(await blockDB.get(blockNumber.toString())),
            ]);
  ...
  fastify.listen(PORT, (err, address) => {
    if (err) {
      console.log(
        `\x1b[31mERROR\x1b[0m [${new Date().toISOString()}] Error at API server: Fastify: `,
        err
      );
      process.exit(1);
    }
    ...
    console.log(
      `\x1b[32mLOG\x1b[0m [${new Date().toISOString()}] API server listening on PORT ${PORT}`
    );
  });
}
